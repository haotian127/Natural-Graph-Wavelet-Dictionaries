...
function gplot(A::SparseMatrixCSC{Float64,Int}, xyz::Matrix{Float64}; plotp::Bool = true,
    ...
    nodes_Cartesian = findall(!iszero, A)
    i = [nodes_Cartesian[k][1] for k=1:length(nodes_Cartesian)]
    j = [nodes_Cartesian[k][2] for k=1:length(nodes_Cartesian)]
    p = sortperm(max.(i,j))
    i = i[p]
    j = j[p]
...
function gplot!(A::SparseMatrixCSC{Float64,Int}, xyz::Matrix{Float64}; plotp::Bool = true,
    ...
    # Extract the node indices
    nodes_Cartesian = CartesianIndices(size(A))[LinearIndices(A)[findall(!iszero, A)]]
    i = [nodes_Cartesian[k][1] for k=1:length(nodes_Cartesian)]
    j = [nodes_Cartesian[k][2] for k=1:length(nodes_Cartesian)]
    p = sortperm(max.(i,j))
    i = i[p]
